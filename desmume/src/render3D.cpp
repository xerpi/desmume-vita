...
Render3DError Render3D::TextureUpscale(const NDSTextureFormat texFormat, const u32 *src, size_t &outTexWidth, size_t &outTexHeight)
{
	...
	{
		xbrz::scale<SCALEFACTOR, xbrz::ColorFormatARGB>((const uint32_t *)src, (uint32_t *)this->_textureUpscaleBuffer, outTexWidth, outTexHeight);
	}
	else
	{
		xbrz::scale<SCALEFACTOR, xbrz::ColorFormatARGB_1bitAlpha>((const uint32_t *)src, (uint32_t *)this->_textureUpscaleBuffer, outTexWidth, outTexHeight);
	}
